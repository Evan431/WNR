...
def recupSerie(id):
    ...
    if len(details.videos['results']) > 0:
        video = details.videos['results'][0]['key']
    else:
        video = False

    serie = Serie.objects.get_or_create(titre=details.name, titreOriginal=details.original_name,
                                        popularite=details.popularity, description=details.overview, date=details.first_air_date, affiche=details.poster_path, dureeMoyEp=runtime, bandeAnnonce=video, nombreSaison=details.number_of_seasons, nombreEpisodes=details.number_of_episodes, status=details.status)[0]
